import { memo, forwardRef } from 'react';
import { IconBase } from '../base.js';
import type { IconBaseProps } from '../base.js';

type BatteryFullBoldDuotoneProps = Omit<IconBaseProps, 'children'>;

const BatteryFullBoldDuotone = memo(
  forwardRef<SVGSVGElement, BatteryFullBoldDuotoneProps>((props, ref) => (
    <IconBase ref={ref} iconName="battery-full-bold-duotone" {...props}>
      <path fill="currentColor" d="M16.4 8q.4 0 .74.02.36.01.77.2a2 2 0 0 1 .8.73l.07.14.09.2q.1.3.11.57.02.34.02.74v2.8q0 .4-.02.74-.01.36-.2.77a2 2 0 0 1-.87.87q-.41.19-.77.2-.34.02-.74.02H5.6q-.4 0-.74-.02-.35-.01-.77-.2a2 2 0 0 1-.87-.87 2 2 0 0 1-.2-.77q-.03-.34-.02-.74v-2.8q0-.4.02-.74.01-.36.2-.77a2 2 0 0 1 .87-.87c.27-.14.54-.18.77-.2Q5.2 8 5.6 8z" />
        <path fill="currentColor" fillRule="evenodd" d="M15 5q1.37-.01 2.26.04.9.04 1.65.34a4.8 4.8 0 0 1 3.02 4.12h.57c.83 0 1.5.67 1.5 1.5v2c0 .83-.67 1.5-1.5 1.5h-.56a4.8 4.8 0 0 1-4.68 4.46q-.89.05-2.26.04H7q-1.37.01-2.26-.04a5 5 0 0 1-1.65-.34 5 5 0 0 1-2.7-2.7 5 5 0 0 1-.35-1.66Q-.01 13.37 0 12q-.01-1.37.04-2.26.04-.9.34-1.65a5 5 0 0 1 2.7-2.7c.52-.22 1.05-.3 1.66-.35Q5.63 4.99 7 5zM7 7c-.95 0-1.6 0-2.12.04-.5.03-.8.1-1.03.19a3 3 0 0 0-1.62 1.62c-.1.23-.16.52-.2 1.03C2 10.4 2 11.05 2 12s0 1.6.04 2.12c.03.5.1.8.19 1.03a3 3 0 0 0 1.62 1.62c.23.1.52.16 1.03.2C5.4 17 6.05 17 7 17h8c.95 0 1.6 0 2.12-.04.5-.03.8-.1 1.03-.19a3 3 0 0 0 1.62-1.62c.1-.23.16-.52.2-1.03.03-.52.03-1.17.03-2.12s0-1.6-.04-2.12c-.03-.5-.1-.8-.19-1.03a3 3 0 0 0-1.62-1.62c-.23-.1-.52-.16-1.03-.2C16.6 7 15.95 7 15 7z" clipRule="evenodd" opacity={.4} />
    </IconBase>
  ))
);

BatteryFullBoldDuotone.displayName = 'BatteryFullBoldDuotone';

// Triple export pattern (lucide-react style)
export { BatteryFullBoldDuotone, BatteryFullBoldDuotone as BatteryFullBoldDuotoneIcon, BatteryFullBoldDuotone as SiBatteryFullBoldDuotone };
export default BatteryFullBoldDuotone;
export type { BatteryFullBoldDuotoneProps };
