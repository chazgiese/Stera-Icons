import { memo, forwardRef } from 'react';
import { IconBase } from '../IconBase';
import type { IconBaseProps } from '../IconBase';

type Clock10RegularDuotoneProps = Omit<IconBaseProps, 'children'>;

const Clock10RegularDuotone = memo(
  forwardRef<SVGSVGElement, Clock10RegularDuotoneProps>((props, ref) => (
    <IconBase ref={ref} {...props}>
      <path fill="currentColor" fillRule="evenodd" d="M12 1.25a10.75 10.75 0 1 1 0 21.5 10.75 10.75 0 0 1 0-21.5m0 1.5a9.25 9.25 0 1 0 0 18.5 9.25 9.25 0 0 0 0-18.5" clipRule="evenodd" opacity={.4} />
        <path fill="currentColor" d="M12 5.25c.41 0 .75.34.75.75v6q0 .1-.03.2l-.02.07-.02.03-.03.07-.02.03-.15.17-.04.04-.08.04-.05.03-.14.05-.11.02h-.17l-.08-.03-.1-.03-.09-.04-3.46-2a.75.75 0 1 1 .75-1.3l2.34 1.35V6c0-.41.34-.75.75-.75" />
    </IconBase>
  ))
);

Clock10RegularDuotone.displayName = 'Clock10RegularDuotone';

export { Clock10RegularDuotone };
export type { Clock10RegularDuotoneProps };
